...
//     vec3 color = base * clamp(ambient + diffuse, 0.0, 1.5);
...
//     // -----------------------------
//     // Lighting vectors (view space)
//     // -----------------------------
//     vec3 N = normalize(nrm);
//     vec3 L = normalize(kLightDirView);
//     vec3 V = normalize(-pos);              // view direction (camera at origin in view space)
...
//     // -----------------------------
//     float ambient = 0.20;
//     vec3 diffuseCol = base * (ambient + NdotL);

//     // -----------------------------
//     // Simple specular (roughness + metallic stepping stone)
//     // - roughness controls shininess
//     // - metallic tints specular toward base color
//     // -----------------------------
//     float rough = clamp(mat.roughness, 0.04, 1.0);
//     float metal = clamp(mat.metallic, 0.0, 1.0);

//     // Map roughness -> exponent (smooth = high exponent)
//     float specPow = mix(8.0, 256.0, 1.0 - rough);

//     // "F0" approximation: dielectrics ~0.04, metals use base color
//     vec3 F0 = mix(vec3(0.04), base, metal);

//     // Blinn-Phong lobe with a little normalization-ish scaling
//     float specLobe = pow(NdotH, specPow);

//     // Fade spec at grazing angles / avoid exploding when NdotV is tiny
//     float visibility = smoothstep(0.0, 0.2, NdotV);

//     vec3 specCol = F0 * specLobe * visibility;

//     // Reduce diffuse as metallic increases (metal has little/no diffuse)
//     diffuseCol *= (1.0 - metal);

//     // -----------------------------
//     // Emissive (material only for now)
//     // -----------------------------
...
//     // Combine
//     // -----------------------------
//     vec3 color = diffuseCol + specCol + emissive;
