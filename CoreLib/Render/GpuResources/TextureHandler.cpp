...
namespace
{
                             ...
                             uint32_t        mipLevels)
    {
        ...
        else if (oldLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && newLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
        {
            ...
            // If RT raygen samples these, you can broaden this later, but this is OK for now.
    ...
    m_imageHandler(imageHandler)
{
    ...
    // Fallback is required to safely update a combined image sampler array.
    // If this fails, renderer-side updates should detect null fallback and skip.
    m_hasFallback = createFallbackTexture();
}
...
bool TextureHandler::createFallbackTexture() noexcept
{
    ...
    if (!createStagingBuffer(m_ctx, 4, stagingBuffer, stagingMemory))
        return false;

    void* mapped = nullptr;
    if (vkMapMemory(m_ctx.device, stagingMemory, 0, 4, 0, &mapped) != VK_SUCCESS)
    {
    ...
    vkutil::GpuImage gpuImg =
        vkutil::createDeviceLocalImage2D(m_ctx, 1, 1, mipLevels, format, usage);
    ...
    m_fallback             = {};
    m_fallback.image       = gpuImg.image;
    m_fallback.view        = view;
    m_fallback.memory      = gpuImg.memory;
    m_fallback.sampler     = sampler;
    m_fallback.width       = gpuImg.width;
    m_fallback.height      = gpuImg.height;
    m_fallback.mipLevels   = gpuImg.mipLevels;
    m_fallback.format      = gpuImg.format;
    m_fallback.sourceImage = kInvalidImageId;
                                                ...
                                                const std::string& debugName)
{
    ...
    if (img->isKtx())
    {
        ...
        // -----------------------------------------------------
        // Copy each mip level using the KTX-provided offsets/sizes.
        // Final layout must be SHADER_READ_ONLY_OPTIMAL.
        // -----------------------------------------------------
        ...
        for (uint32_t level = 0; level < mipLevels; ++level)
        {
            ...
            r.imageOffset                     = {0, 0, 0};
            r.imageExtent                     = {
                std::max(1u, static_cast<uint32_t>(ml.width)),
                std::max(1u, static_cast<uint32_t>(ml.height)),
                1u};
    ...
    // ---------------------------------------------------------
    // Raw pixels path (your existing implementation)
    // ---------------------------------------------------------
    const int      width    = img->width();
    const int      height   = img->height();
...
void TextureHandler::destroyTexture(GpuTexture& tex) noexcept
{
    ...
    tex.sourceImage = kInvalidImageId;
}
